...
# To create a comment, you need to first create a post
@pytest.fixture()
...
async def test_create_post_expired_token(async_client: AsyncClient, confirmed_user: dict, mocker):
    ...
    response = await async_client.post(
        "/post?prompt=A dog on the couch",
        json={"body": "Test Post"},
...
async def test_get_all_posts_sorting(
    ...
    logged_in_token: str,  # created_post: dict could be usesd instread, but i need at least 2 posts to sort
    sorting: str,
    expected_order: list[int],
):
    await create_post("Test Post 1", async_client, logged_in_token)  # post id 1
    await create_post("Test Post 2", async_client, logged_in_token)  # post id 2
...
async def test_get_all_posts_sort_likes(
    ...
    data = response.json()
    post_ids = [post["id"] for post in data]
    assert post_ids == [1, 2]  # await like_post(2, ...) would result [2, 1] (default order)


@pytest.mark.anyio
async def test_get_all_posts_wrong_sorting(async_client: AsyncClient):  # no need other params

...
async def register(user: UserIn, background_tasks: BackgroundTasks, request: Request):
    if await get_user(user.email):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,  # starlette site-package
    ...
    background_tasks.add_task(
        ...
        user.email,
        confirmation_url=request.url_for(  # generate a url for the confirmation endpoint
            "confirm_email", token=create_confirmation_token(user.email)
        ),
    )
    return {"detail": "User created. Check your email for a confirmation link."}
